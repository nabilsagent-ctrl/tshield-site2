...
## No brown — 2026-09-07 (TOPAZ-971)
- Nabil, on opening the Claude Design project: "Make sure you don't put in the brown, please." The whole `.tsd--warm` W5 palette (ink #2B211B, muted #7A6C5E, mid #BFB1A0, beige surfaces, hero #2b241f, `sepia(.16)` photo filter, footer #E7DCCB) was replaced by a soft NEUTRAL light scale (ground #F8F7F4 · surface #EFEDE8 · surface-2 #E8E6E1 · hairline #E3E0DA · mid #B9B7B3 · muted #7A7772 · ink-2 #3E3C39 · ink #2A2A2A; band/hero graphite; photo filter none) — the same scale the corrected Franchise Guide PDF uses. Every value < 4% saturation.
- Temperature renamed `warm` → **`light`** (`ThemeScope temperature="light"`, `.tsd--light`); `warm` stays as a deprecated alias that renders the identical neutral palette, so nothing built against 0.1.0 breaks. `cfg.provider.props.temperature` is now `light`.
- If a preview or custom CSS ever needs a colour that is not a token, it is wrong — add a token.
